...
#### Flujo de Peticiones y Casos de Uso

Este diagrama ilustra cómo las solicitudes que llegan a `ms-services` se procesan internamente.

<Image src="/images/blogs/ecosystem/ms-services/ms-service-flow-diagram.svg" alt="Diagrama de Flujo CQRS de un microservicio" width="2550" height="1314" decoding="async" loading="lazy"/>
...
<Image src="/images/blogs/ecosystem/ms-services/ms-service-ddd-diagram.svg" alt="Diagrama de un Agregado de Dominio" width="2550" height="1314" decoding="async" loading="lazy"/>
...
<Aside title="Profundiza en la Arquitectura">
Este flujo, que separa `Commands` y `Queries` (CQRS) y modela la lógica de negocio con `Agregados` (DDD), es el estándar para **todos** los microservicios del ecosistema. Si quieres entender a fondo estos patrones, te recomendamos leer nuestro artículo detallado sobre el **Arquetipo de CodeDesignPlus**.
...
En nuestro [artículo anterior](/blog/ecosistema/2-recolectando-secretos-globales), recolectamos toda la información. Ahora, almacenaremos solo la parte confidencial en Vault.

1.  **Ejecutar el Comando `vault kv put`**
    Abre una terminal con la CLI de Vault autenticada y ejecuta el siguiente comando para guardar las credenciales.
    ...
    ```
    <Image src="/images/blogs/ecosystem/ms-services/vault-put.png" alt="Configurando los secretos confidenciales para ms-services en Vault" width="2550" height="1314" decoding="async" loading="lazy"/>
...
1.  **Desplegando el Entrypoint REST (`ms-services-rest`)**
    ...
    *   **Preparar el `values-rest.yaml` Local**
        Creamos un archivo `values-rest.yaml` para gestionar la configuración.
        ...
        ```
        <Aside title="La Jerarquía de Configuración de .NET y CodeDesignPlus">
        Esta configuración híbrida es posible gracias al potente sistema de configuración de .NET y a la inteligencia del SDK de CodeDesignPlus. El orden de precedencia es el siguiente:
        1.  **Base (`appsettings.json`):** El microservicio se inicia con los valores por defecto definidos en su archivo `appsettings.json`.
        2.  **Variables de Entorno (Sobrescritura):** .NET busca automáticamente variables de entorno que coincidan con la estructura de la configuración. La sintaxis `SECTION__SUBSECTION` se traduce a `Section:Subsection`. Por ejemplo, la variable `RABBITMQ__HOST` sobrescribe el valor de `RabbitMQ:Host` en el `appsettings.json`. Usamos esto para configuraciones **no sensibles**.
        3.  **Vault (Sobrescritura Final y Segura):** Finalmente, el SDK de CodeDesignPlus se conecta a Vault y lee los secretos. Estos valores **sobrescriben cualquier configuración previa**, asegurando que los datos confidenciales (como `RabbitMQ:Password`) provengan siempre de la fuente más segura y nunca estén expuestos.
        </Aside>
...
2.  **Desplegando el Entrypoint gRPC (`ms-services-grpc`)**
    ...
    *   **Preparar el `values-grpc.yaml` Local**
        El `values-grpc.yaml` usa la misma lógica de variables de entorno y conexión a Vault, pero omite la configuración del `VirtualService` ya que no se expone a Internet.
