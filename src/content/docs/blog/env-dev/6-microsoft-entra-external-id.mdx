...
30. **Habilitar el Mapeo de los Claims**

    Finalmente, vamos a la opción `Manifest` y buscamos la propiedad `acceptMappedClaims`. La cambiamos a `true` y guardamos. Esto asegura que los claims personalizados que hemos configurado se incluyan en los tokens emitidos y asi evitar el error  `AADSTS50146`.
